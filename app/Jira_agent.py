...
# # Replace the agent-based flow (agent_executor.invoke) with this function
# def generate_release_notes_from_query(user_query: str) -> dict:
#     """
#     Sequential pipeline: extract project/version, run tool functions, save PDF.
#     Works without AgentExecutor / agent changes.
#     """
#     extracted = llm_extract_project_and_version(user_query)
#     if not extracted:
#         return {"reply": "I can only generate release notes if you provide a JIRA project and version."}
...
#     version_info = json.loads(version_info_raw) if isinstance(version_info_raw, str) else version_info_raw
...
#         return {"reply": f"Error fetching JIRA issues: {issues_raw}"}
